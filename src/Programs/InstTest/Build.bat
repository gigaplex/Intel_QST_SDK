...
rem                                                                       rem
rem   File Name:     Programs\InstTest\Build.bat                          rem
rem                                                                       rem
rem   Description:   Builds Windows  and  DOS  executables  for  sample   rem
rem                  program   InstTest,   which  demonstrates  how  to   rem
rem                  utilize the services of the Instrumentation  Layer   rem
rem                  (IL) for Intel(R) Quiet System Technology (QST).     rem
rem                                                                       rem
rem remremremremremremremremremremremremremremremremremremremremremremrem rem
...
echo ******************************************************************* >Build.log
msdev "InstTest.dsp" /make "InstTest - Win32 Release" /rebuild >>Build.log
if errorlevel 1 goto WINRFAIL

echo ******************************************************************* >>Build.log
msdev "InstTest.dsp" /make "InstTest - Win32 Debug" /rebuild >>Build.log
...
%WATCOM%\binnt\wcc386 InstTest.c /fo=%TEMPDIR%\InstTest.obj %MODELOPTS% %OPTS% >>Build.log
...
echo file       %TEMPDIR%\InstTest                      >>%TEMPDIR%\Build.lnk
...
echo name       %TEMPDIR%\InstTest.exe                  >>%TEMPDIR%\Build.lnk
echo option     map=%TEMPDIR%\InstTest.map              >>%TEMPDIR%\Build.lnk
...
%WATCOM%\binnt\wdis %TEMPDIR%\InstTest /l=%TEMPDIR%\InstTest.lst /s=InstTest.c >>Build.log
if errorlevel 1 goto ERROR
